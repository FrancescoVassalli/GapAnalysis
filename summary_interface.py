...
from models import ChatHistory, Bait
...
def summarize(session:Session, baits:Collection[int]=None):
    ...
    if baits is None:
        baits = session.exec(select(Bait.id)).all()
    chat_paragraphs = [add_interview_header(collect_chats_in_paragraph_format(session,bait_id),bait_id) for bait_id in baits]
